...
struct nmap nmap = {
	.scans = { true, true, true, true, true, true }, // all scans enabled by default
	.threads = 0, // number of threads
	.port_start = 1, // port start
	.port_end = 1024, // port end
	.target_opt = NULL, // target_opt (-t argument)
	.target_file = NULL, // target_file (-f argument)
	.target_arg = NULL, // target_arg (non option argument)
	.spoofed_source = {0} // spoofed source
};

struct ports	ports;
int				send_sock;
struct target	*targets = NULL; // array of targets with name and ip address
struct task		*tasks = NULL; // liste chaînée
struct result	*results = NULL; // array of results per target
size_t			nb_results = 0;
pthread_mutex_t	task_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t	result_mutex = PTHREAD_MUTEX_INITIALIZER;
struct itimerspec	timeout_delay =
{
	.it_interval = { .tv_sec = 0, .tv_nsec = 0 },
	.it_value = { .tv_sec = 0, .tv_nsec = INITIAL_RTT_TIMEOUT * 1000000 }
};
...
void free_targets(struct target *targets)
{
	struct target *next;
	while (targets)
	{
...
int main(int argc, char **argv)
{
	const struct argp_option options[] = {
		...
		{"threads", 'm', "THREADS", 0, "maximum number of threads to use for the scan (default: 0) (max: 250)", 0},
		{"scan", 's', "TYPE", 0, "type of scan to use, must be one of SYN, NULL, ACK, FIN, XMAS, UDP (all used if not specified)", 0},
		{"timeout", 'T', "TIMEOUT", 0, "timeout in milliseconds for each probe (default: 100)", 0},
